 ...
 * `href` is a `data:` URL. The PNG bytes are Base64-encoded in Kotlin so only `String` values cross
 * the WasmJs interop boundary.
 *
...
private fun downloadDataUrl(
    url: String,
    name: String,
): Unit = js("{ var a = document.createElement('a'); a.href = url; a.download = name; a.click(); }")
